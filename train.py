...
def train(cfg: DictConfig) -> None:
    ...
    # initialize wandb
    with (wandb.init(project="DQN", name=f"{cfg.environment.environment_id}_{seed}", config=wandb_config) as run):
        # log network gradients
        run.watch(agent.online_net, log_freq=100)
        print("Training...")
        ...
        while curr_step < (cfg.training.train_steps // (agent.batch_size // 32)):
            ...
            if curr_step % cfg.training.report_frequency == 0:
                ...
                run.log({"training_reward": np.mean(mean_ep_reward),
                         "training_length": np.mean(mean_ep_length),
                         "training_loss": np.mean(mean_ep_loss),
                         "training_q": np.mean(mean_ep_q)})
            ...
            if curr_step % (cfg.training.test_frequency // (agent.batch_size // 32)) == 0 and curr_step > cfg.training.burn_in_steps:
                ...
                std = np.std(metrics)
                run.log({"Test Reward (mean)": mean, "Test Reward (std)": std})
        ...
        model_artifact = wandb.Artifact("best_agent", type="model", description="best agent")
